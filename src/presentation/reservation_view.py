...
"""
   Class representing the GUI window for managing reservations.
   ...
   Methods:
       ...
       delete_reservation(self): Deletes the selected reservation.
       move_back(self): Closes the current window and moves back to the previous window.
   """
class Reservation_view(tk.Toplevel):
    def __init__(self, master, app):
        super().__init__(master)
        self.app = app
        self.reservations = self.master.app.load_reservations()
        self.log_writter = Log_writter()
        self.setup()

    def setup(self):
        # setting title
        self.title("Bernard's Airport - Reservations")
        # setting window size
        width = 1850
        height = 700
        ...
        self.label_heading["text"] = "Reservations"
        self.label_heading.place(x=750, y=40, width=300, height=100)

        self.flight_frame = ttk.Frame(self)
        self.flight_frame.place(relx=0.5, rely=0.4, anchor=tk.CENTER)

        self.flight_tree = ttk.Treeview(self.flight_frame, columns=("Fly number", "Passenger", "Country", "Plane", "Pilot", "Date leaving", "Date arriving", "Price"))
        self.flight_tree.heading("#0", text="PIN")
        self.flight_tree.heading("Fly number", text="Fly number")
        self.flight_tree.heading("Passenger", text="Passenger")
        ...
        for reservation in self.reservations:
            self.flight_tree.insert("", "end", text=reservation[0],
                                    values=(reservation[1], reservation[2], reservation[3], reservation[4], reservation[5] ,reservation[6], reservation[7], reservation[8]))
    ...
    def delete_reservation(self):
        ...
        if len(selected_items) == 1:
            ...
            if values:
                if self.master.app.reservation_dao.delete(values):
                    self.destroy()
